...
func (dao *UserDao) IsExistByEmail(email string) bool {
	var count int64
	if dao.db.Model(&model.Customer{}).Where("email = ?", email).Count(&count); count == 1 {
		return true
	}
	return false
}
...
func (dao *UserDao) UpdatePassword(username string, password string) error {
	return dao.db.Model(&model.Customer{}).Where("username = ?", username).Update("password", password).Error
}

func (dao *UserDao) IsExistByNickName(name string) bool {
	var count int64
	if dao.db.Model(&model.Customer{}).Where("username = ?", name).Count(&count); count == 1 {
		return true
	}
	return false
}
...
func (dao *UserDao) UpdateUserName(name string, newName string) error {
	return dao.db.Model(&model.Customer{}).Where("username = ?", name).Update("username", newName).Error
}
...
func (dao *UserDao) GetUserByName(name string) *model.Customer {
	u := &model.Customer{}
	dao.db.Where("email = ?", name).Find(u)
	return u
}
